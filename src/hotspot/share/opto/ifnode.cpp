...
Node* IfNode::search_identical(int dist) {
  ...
  int op = Opcode();
#if INCLUDE_SHENANDOAHGC
  bool heap_stable_test = ShenandoahWriteBarrierNode::is_heap_stable_test(this);
#endif
  // Search up the dominator tree for an If with an identical test
  while (dom->Opcode() != op    ||  // Not same opcode?
         (dom->in(1) != in(1) SHENANDOAHGC_ONLY(&& (!heap_stable_test || !ShenandoahWriteBarrierNode::is_heap_stable_test(dom->as_If())))) ||  // Not same input 1?
